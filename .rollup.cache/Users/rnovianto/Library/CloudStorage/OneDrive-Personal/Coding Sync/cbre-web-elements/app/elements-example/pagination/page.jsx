...
export default function PaginationExamplePage() {
    ...
    const generatePaginationItems = () => {
        ...
        <PaginationPrevious onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} aria-disabled={currentPage === 1} tabIndex={currentPage === 1 ? -1 : 0} className={currentPage === 1 ? "pointer-events-none opacity-50" : ""}/>
      </PaginationItem>);
        // Show ellipsis at start if needed
        if (startPage > 1) {
            items.push(<PaginationItem key="start">
        ...
        if (endPage < totalPages) {
            ...
            items.push(<PaginationItem key="end">
        ...
        <PaginationNext onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} aria-disabled={currentPage === totalPages} tabIndex={currentPage === totalPages ? -1 : 0} className={currentPage === totalPages ? "pointer-events-none opacity-50" : ""}/>
      </PaginationItem>);
        return items;
    };
            ...
            <div className="border border-light-grey bg-white p-8 max-w-2xl mx-auto">
              <div className="flex flex-col items-center space-y-6">
                <Pagination>
                  <PaginationContent>
                    <PaginationItem>
                      <PaginationPrevious onClick={() => setBasicPage(prev => Math.max(1, prev - 1))} aria-disabled={basicPage === 1} tabIndex={basicPage === 1 ? -1 : 0} className={basicPage === 1 ? "pointer-events-none opacity-50" : ""}/>
                    </PaginationItem>
                    ...
                    <PaginationItem>
                      <PaginationNext onClick={() => setBasicPage(prev => Math.min(basicTotalPages, prev + 1))} aria-disabled={basicPage === basicTotalPages} tabIndex={basicPage === basicTotalPages ? -1 : 0} className={basicPage === basicTotalPages ? "pointer-events-none opacity-50" : ""}/>
                    </PaginationItem>
                  </PaginationContent>
                </Pagination>
          ...
          <div className="bg-[var(--lighter-grey)] p-4 md:p-8">
            <div className="border border-light-grey bg-white p-8 max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                {currentItems.slice(0, 6).map(item => (<div key={item.id} className="p-4 bg-[var(--lighter-grey)] border border-light-grey rounded-md">
