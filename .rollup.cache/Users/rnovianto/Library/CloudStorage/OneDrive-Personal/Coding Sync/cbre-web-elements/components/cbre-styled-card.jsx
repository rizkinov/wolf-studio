 ...
 */
export function CBREStyledCard({ title, description, children, className, headerClassName, footerClassName, accentColor = 'default', footerAction }) {
    ...
    return (<Card className={cn("flex flex-col border-t-4", accentColorMap[accentColor], className)}>
      ...
      {footerAction && (<CardFooter className={footerClassName}>
          <Button variant="ghost" size="sm" className="text-cbre-green p-0 hover:bg-transparent hover:text-accent-green" onClick={footerAction.onClick}>
            {footerAction.label}
          </Button>
        </CardFooter>)}
    </Card>);
}
//# sourceMappingURL=cbre-styled-card.jsx.map