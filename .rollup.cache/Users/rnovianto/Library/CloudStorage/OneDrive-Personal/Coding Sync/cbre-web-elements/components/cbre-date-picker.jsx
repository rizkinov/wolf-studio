 ...
 */
export function CBREDateRangePicker({ dateRange, setDateRange, label, description, placeholder = "Select date range", error, disabled = false, className, disabledDates, numberOfMonths = 2, }) {
    const [date, setDate] = React.useState(dateRange);
    const handleDateSelect = React.useCallback((selectedDateRange) => {
        setDate(selectedDateRange);
        setDateRange === null || setDateRange === void 0 ? void 0 : setDateRange(selectedDateRange);
    }, [setDateRange]);
    React.useEffect(() => {
        setDate(dateRange);
    }, [dateRange]);
    return (<div className={cn("space-y-2", className)}>
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !date && "text-muted-foreground", error && "border-destructive")} disabled={disabled}>
            <CalendarIcon className="mr-2 h-4 w-4"/>
            {(date === null || date === void 0 ? void 0 : date.from) ? (date.to ? (<>
                  {formatDate(date.from)} - {formatDate(date.to)}
                </>) : (formatDate(date.from))) : (placeholder)}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0" align="start">
          <Calendar initialFocus mode="range" defaultMonth={date === null || date === void 0 ? void 0 : date.from} selected={date} onSelect={handleDateSelect} numberOfMonths={numberOfMonths} disabled={disabled || disabledDates}/>
        </PopoverContent>
      </Popover>
      {error && (<p className="text-destructive text-sm font-medium">{error}</p>)}
    </div>);
}
//# sourceMappingURL=cbre-date-picker.jsx.map